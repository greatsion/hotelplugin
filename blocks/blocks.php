...
class HBookBlocks {
	...
	public function block_editor_assets() {
		$blocks = array( 'booking-form', 'accom-list', 'availability', 'rates', 'resa-summary', 'starting-price', 'payment-link-form' );
		...
		$text = array(
			...
			'payment_link_form_title' => esc_html__( 'Payment link form', 'hbook-admin' ),
			'payment_link_form_settings' => esc_html__( 'Payment link form settings', 'hbook-admin' ),
			'payment_link_form_block' => esc_html__( 'Payment link form block.', 'hbook-admin' ),
		);
	...
	public function register_blocks() {
		...
		register_block_type(
			'hbook/payment-link-form',
			array(
				'render_callback' => array( $this, 'payment_link_form_block_render' ),
				'attributes' => array(
					'thank_you_page_id' => array(
						'type' => 'string',
						'default' => 'none'
	...
	public function payment_link_form_block_render( $attributes ) {
		if ( defined( 'REST_REQUEST' ) || is_admin() ) {
			return;
		}
		require_once $this->utils->plugin_directory . '/front-end/renders/hbook-render.php';
		require_once $this->utils->plugin_directory . '/front-end/renders/payment-link-form-render.php';
		$payment_link_form = new HBookPaymentLinkForm( $this->hbdb, $this->utils );
		$attributes['thank_you_page_url'] = '';
		if ( $attributes['thank_you_page_id'] != 'none' ) {
			$attributes['thank_you_page_url'] = get_permalink( $attributes['thank_you_page_id'] );
		}
		return $payment_link_form->render( $attributes );
	}
