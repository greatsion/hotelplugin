...
class HbDetailsForm {
	...
	private function get_coupon_area() {
		...
		if ( $this->hbdb->site_has_coupons() ) {
			...
			$output .= '<input type="hidden" name="hb-pre-validated-coupon-id" class="hb-pre-validated-coupon-id" value="" />';
			...
			$output .= '</p>';
			$output .= '<p class="hb-processing-coupon hb-dots-loader">' . str_replace( '...', '<span></span>', $this->hb_strings[ 'verifying_coupon' ] ) . '</p>';
	...
	private function get_payment_fields() {
		...
		$output .= $this->get_payment_methods();
	...
	public function get_payment_methods( $included_payment_gateways = false ) {
		$output = '<div class="hb-payment-method-wrapper">';

		if ( ! $included_payment_gateways ) {
			$payment_gateways = $this->utils->get_active_payment_gateways();
		} else {
			$payment_gateways = array();
			foreach ( $included_payment_gateways as $gateway_id ) {
				$payment_gateways[] = $this->utils->get_payment_gateway( $gateway_id );
			}
		}
		...
		$output .= '</div><!-- end .hb-payment-method-wrapper -->';
	...
	private function get_resa_summary( $is_admin ) {
		$output = '<p class="hb-loading-summary hb-dots-loader">';
		if ( $is_admin ) {
			$output .= '<span class="spinner"></span>';
			$output .= esc_html__( 'Loading summary...', 'hbook-admin' );
		} else {
			$output .= str_replace( '...', '<span></span>', esc_html( $this->hb_strings['summary_loading'] ) );
		}
		$output .= '</p>';
	...
	private function get_admin_details_fields() {
		...
		$output .= '<p class="hb-admin-add-resa-section-title">';
		$output .= '<label for="hb-admin-comment">';
		$output .= esc_html__( 'Status:', 'hbook-admin' );
		$output .= '</label>';
		$output .= '</p>';

		$output .= '<div>';
		$output .= '<input type="radio" id="hb-new-resa-status-pending" name="hb-new-resa-status" value="pending" />';
		$output .= ' <label for="hb-new-resa-status-pending">' . esc_html__( 'Pending', 'hbook-admin' ) . '</label><br/>';
		$output .= '<input type="radio" id="hb-new-resa-status-new" name="hb-new-resa-status" value="new" />';
		$output .= ' <label for="hb-new-resa-status-new">' . esc_html__( 'New', 'hbook-admin' ) . '</label><br/>';
		$output .= '<input type="radio" id="hb-new-resa-status-confirmed" name="hb-new-resa-status" value="confirmed" />';
		$output .= ' <label for="hb-new-resa-status-confirmed">' . esc_html__( 'Confirmed', 'hbook-admin' ) . '</label><br/>';
		$output .= '</div>';
	...
	private function get_confirm_area() {
		...
		'<div class="hb-confirm-area">' .
			'<p class="hb-saving-resa hb-dots-loader">' . str_replace( '...', '<span></span>', $this->hb_strings['processing'] ) . '</p>' .
			$txt_before_book_now_button .
