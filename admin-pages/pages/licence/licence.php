...
class HbAdminPageLicence extends HbAdminPage {
	...
	<div class="wrap">
		...
		<p><a href="https://maestrel-resource-center.refined.site/space/KB/33030606/"><?php esc_html_e( 'Where can I find my purchase code?', 'hbook-admin' ); ?></a></p>
				...
				if ( get_option( 'hb_purchase_code_error' ) == 'no-online-validation' ) {
					...
					if ( strpos( $error_text, '%s' ) ) {
						$error_text = str_replace(
							'%s',
							'<a target="_blank" href="https://maestrel-resource-center.refined.site/space/KB/61735043/">' .
							esc_html__( 'this article of the knowledgebase', 'hbook-admin' ) .
							'</a>', $error_text
						);
					} else {
						$error_text .= ' - <a target="_blank" href="https://maestrel-resource-center.refined.site/space/KB/61735043/">https://maestrel-resource-center.refined.site/space/KB/61735043/</a>';
					}
					...
					if ( strpos( $error_text, '%s' ) ) {
						$error_text = str_replace( '%s', '<a target="_blank" href="https://maestrel.atlassian.net/servicedesk/customer/portal/3/group/9/create/63/">' . $hwp_support_string . '</a>', $error_text );
					} else {
						$error_text .= ' - <a target="_blank" href="https://maestrel.atlassian.net/servicedesk/customer/portal/3/group/9/create/63/">Maestrel support</a>';
					}
				}
				?>
		...
		foreach ( $gateway_list as $gateway => $data ) {
			if ( ( 'paypal' != $data->id ) && ( 'stripe' != $data->id ) ) {
				?>
				<hr/>
				<h3><?php printf( esc_html__( 'Activate your %s licence', 'hbook-admin' ), esc_html( $data->name ) ); ?></h3>
				<?php
				...
				<p>
					...
					<input type="hidden" name ="hb-addon-name" value="<?php echo( esc_html( $data->id ) ); ?>" />
					<input type="submit" value="<?php esc_attr_e( 'Verify purchase code', 'hbook-admin' ) ?>" class="button-primary" />
				</p>
