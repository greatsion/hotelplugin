...
class HbAdminPagePayment extends HbAdminPage {
	...
	public function __construct( $page_id, $hbdb, $utils, $options_utils ) {
		...
		$this->data = array(
			'hb_text' => array(
				...
				'deposit_percentage' => esc_html__( 'Deposit percentage', 'hbook-admin' ),
			),
	...
	<div class="wrap">
		...
		<form id="hb-settings-form">
			...
			$gateway_activate_section = array(
				...
				'desc' => sprintf( esc_html__( 'Stripe and PayPal gateways are included. We have developed add-ons to integrate other payment gateways for HBook: %s view available payment gateways %s.', 'hbook-admin' ), '<a href="https://maestrel.com/payment-gateways" target="_blank">', '</a>' ),
				'options' => array()
			);
			...
			$payment_setting_sections = $this->options_utils->get_payment_settings();
			$payment_link_section = array( 'payment_link' => $payment_setting_sections['payment_link'] );
			unset( $payment_setting_sections['payment_link'] );

			$payment_setting_sections = array_merge(
				$payment_setting_sections,
				array( 'gateway_activate' => $gateway_activate_section ),
				$gateway_options,
			...
			foreach ( $payment_setting_sections as $section_id => $section ) {
				...
				if ( $section_id == 'payment_link' ) {
					$section_class .= ' hb-payment-section-stripe';
				}
				?>
				...
				<?php
				$this->options_utils->display_section_title( 'payment', $section_id, $section['label'] );
				...
				foreach ( $section['options'] as $id => $option ) {
					if ( $id == 'hb_payment_link_page_id' ) {
						$option['choice'] = $this->get_main_language_pages();
					}
	...
	private function get_main_language_pages() {
		$pages = get_pages();
		...
		$returned_pages = array( 0 => '' );
		foreach ( $pages as $p ) {
			if ( function_exists( 'pll_get_post' ) ) {
				if ( pll_get_post_language( $p->ID, 'locale' ) == $default_lang ) {
					$returned_pages[ $p->ID ] = $p->post_title;
				}
			} else if ( function_exists( 'icl_object_id' ) ) {
				$wpml_post_info = wpml_get_language_information( null, $p->ID );
				if ( is_wp_error( $wpml_post_info ) ) {
					$wpml_post_info = wpml_get_language_information( $p->ID );
				}
