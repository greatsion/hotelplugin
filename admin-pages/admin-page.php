...
class HbAdminPage {
	...
	protected function display_ical_email_document_vars() {
		$vars = $this->utils->get_ical_email_document_available_vars();
		$var_type = array(
			'reservation' => esc_html__( 'Reservation variables', 'hbook-admin' ),
			'customer' => esc_html__( 'Customer variables', 'hbook-admin' ),
			'payment' => esc_html__( 'Payment link variables', 'hbook-admin' ),
			'misc' => esc_html__( 'Misc', 'hbook-admin' )
		);
		?>
		<?php
		foreach ( $var_type as $type => $name ) {
			if ( isset( $vars[ $type ] ) ) {
			?>
				<h4><?php echo( esc_html( $name ) ); ?></h4>
				<p><?php echo( esc_html( implode( ' &nbsp;-&nbsp; ', $vars[ $type ] ) ) ); ?></p>
			<?php
			}
		}
	}
